  ...
  const classes =
    priority === "urgent"
      ...
      : priority === "high"
        ...
        : "bg-surface-container text-on-surface-variant";
  const suffix = priority === "urgent" || priority === "high" ? " Prioridade" : "";
  return (
    <span className={`inline-flex h-5 items-center rounded px-2 text-[11px] font-semibold uppercase tracking-[.03em] ${classes}`}>
      {PRIORITY_LABEL[priority]}{suffix}
    </span>
  );
}
...
export function MetricCard({
  ...
  tone?: "critical" | "attention" | "active" | "completed" | "neutral";
  onClick?: () => void;
  to?: string;
}) {
  const classes = tone === "critical"
    ...
    : tone === "attention"
      ? "border-attention-200 bg-attention-100/60"
      : tone === "completed"
...
export function StatCard(props: {
  ...
  const tone =
    props.tone === "danger" ? "critical"
    : props.tone === "success" ? "completed"
    : props.tone === "attention" ? "attention"
    : props.tone === "neutral" ? "neutral"
    : "active";
...
export function EmptyState({ title, hint, action }: { title: string; hint?: string; action?: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-dashed border-outline-variant bg-surface-container-lowest p-8 text-center shadow-tier-1">
      <Check size={22} className="mx-auto mb-2 text-success-600" />
